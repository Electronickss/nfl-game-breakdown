...
add_touch_labels <- function(plot, touch_data, player_team_colors, home_team = NULL) {
  home_touches <- touch_data |> filter(posteam == home_team)
  away_touches <- touch_data |> filter(posteam != home_team)

  if (nrow(home_touches) > 0) home_touches <- get_player_team_colors(home_touches, player_team_colors)
  if (nrow(away_touches) > 0) away_touches <- get_player_team_colors(away_touches, player_team_colors)
    ...
    if (nrow(home_touches) > 0) {
      plot <- plot +
        geom_text(
          data = home_touches,
          aes(x = game_seconds_remaining, y = vegas_home_wp, label = player, color = player),
          size = 2.5, fontface = "bold", vjust = -1.5,
          show.legend = FALSE
        )
    }
    if (nrow(away_touches) > 0) {
...
plot_rb_workload <- function(pbp, week, year, game_id = NULL) {
  ...
  plot <- add_touch_labels(plot, all_touches, logos, home_team = home_team)
