...
load_data <- function(start_year, end_year) {
  nflfastR::load_pbp(start_year:end_year)
}
...
theme_high_contrast <- function(base_size = 11, base_family = "", foreground_color = "white", background_color = "black") {
  ...
  theme(
    line = element_line(colour = foreground_color, linewidth = 0.5, linetype = 1, lineend = "butt"),
    rect = element_rect(fill = foreground_color, colour = foreground_color, linewidth = 0.5, linetype = 1),
    text = element_text(
      family = base_family, face = "plain", colour = foreground_color, size = base_size,
...
plot_win_probability <- function(data, logos, foreground_color = rich_black, background_color = "white") {
  ...
  resolved <- resolve_team_colors(home_team_abbr, away_team_abbr, logos)
  ...
  team_colors <- c(setNames(resolved$home_color, home_team_abbr), setNames(resolved$away_color, away_team_abbr))
  ...
  plot <- ggplot(data, aes(x = game_seconds_remaining, y = vegas_home_wp)) +
    ...
    geom_rug(data = filter(data, home_turnover_play == 1), color = red, sides = "t", linewidth = 0.5) +
    geom_rug(data = filter(data, away_turnover_play == 1), color = red, sides = "b", linewidth = 0.5) +
